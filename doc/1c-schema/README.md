...
В будущем планируется перейти к одной (эталонной) паре регистров версии 1.
...
|----------------------|-------------------------|---------------------|-------------------------------------------------------|
| **МоментВремени**    | Число(19,0)             | numeric(19,0)       | Порядковый номер сообщения                            |
...
| **Получатели**       | Строка(0) переменная    | nvarchar(max)       | Список кодов узлов получателей в формате CSV          |
...
Процедура ОтправитьИсходящееСообщение(СправочникОбъект) Экспорт
	...
	Сообщение.МоментВремени = ТекущаяУниверсальнаяДатаВМиллисекундах();
	...
	Сообщение.Отправитель = "ЦБ";
	Сообщение.Получатели = "N001,N002,N003";
